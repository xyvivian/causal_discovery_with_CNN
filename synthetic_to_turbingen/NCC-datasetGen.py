...
class NCCDataGen(object):
	def __init__(self,ni=15000,saveFile= "",minSize=100,maxSize = 1000):
		"""
	...
	def GMM(self,K,meanSampStd,varSampStd,size):
		...
		means = meanSampStd*np.random.randn(K,1)
		covariates = np.power(abs(varSampStd*np.random.randn(K,1)),2)
		weights = abs(np.random.randn(K,1))
		weights = weights/ sum(weights)

		weight = weights.reshape(K,)


		data = np.zeros((size,1))
		for i in range(size):
			k = np.random.choice(K,size=1, p=weight)[0]
			mean = means[k][0]
			cov =  covariates[k][0]
